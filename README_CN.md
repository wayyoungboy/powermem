...
| Codex app / CLI | Hook 插件，见 [Codex](#codex) |
...
Codex 使用专门的 Codex app hook 插件，不走 MCP。
...
启动新的 Codex 线程，打开 `/hooks`，审查并信任 PowerMem hooks，然后输入：

```text
Use the powermem-codex-plugin init skill to initialize PowerMem.
```

插件会在 `UserPromptSubmit` 时注入相关记忆，并在 `Stop` 时通过 PowerMem HTTP API 保存回合摘要。详见 [docs/integrations/codex.md](docs/integrations/codex.md)。

`init` skill 会用简短对话引导用户选择：连接已有 PowerMem HTTP 集群、启动本地后端，或修复当前配置。它还会询问是否配置 `POWERMEM_USER_ID` 和 `POWERMEM_AGENT_ID` 来决定记忆作用域。本地启动则继续引导用户选择零配置 no-LLM、导入已有 PowerMem `.env`，或手动配置 provider 与凭证。

清理时使用 `uninstall` skill：它按三层递进引导，先删除 Codex hooks/插件，再选择是否删除 marketplace，最后才选择是否删除本地 PowerMem 服务配置。
...
适用于 Claude Desktop、Cline、OpenCode、Roo Code、Goose 或其他兼容 MCP 的客户端。请使用 MCP Client 模式。
