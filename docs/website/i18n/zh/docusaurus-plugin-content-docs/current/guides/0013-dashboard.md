...
### Overview 页面 {#overview-page}

**路径：** `/dashboard/`

Overview 页面提供您的记忆系统的实时分析和系统健康监控。
...
5. **Memory Quality** — 显示质量分析的卡片（见下文）
...
- **Refresh Button** — 手动刷新所有数据
- **Clear Filters** — 当过滤器激活时出现，返回未过滤视图

### Memories 页面 {#memories-page}

**路径：** `/dashboard/memories`

Memories 页面允许您浏览、搜索、过滤和删除存储的记忆记录。
...
3. 检查 **Memory Quality** 卡片（在 Overview 页面）：
...
1. **浏览器缓存：** 点击 Overview 或 Memories 页面上的 **Refresh** 按钮
