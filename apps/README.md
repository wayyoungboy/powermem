...
| **Codex app / Codex CLI** (hook-based plugin) | [`powermem-codex-plugin/`](powermem-codex-plugin/) | `Read and follow apps/powermem-codex-plugin/SETUP.md to setup PowerMem for Codex hooks` |
...
> **Codex** has a dedicated hook plugin. **OpenCode** belongs under `mcp-client/`, not `vscode-extension/`. The VS Code extension flow is for VS Code-compatible IDEs only.
...
| **[powermem-codex-plugin](powermem-codex-plugin/)** | Codex app and Codex CLI plugin that uses Codex lifecycle hooks. No MCP server is required. |
...
The `mcp-client/` path uses `powermem-mcp` directly and prefers SSE on port `8848`. The `vscode-extension/` path prefers the HTTP API and links the current IDE/client first. The `powermem-codex-plugin/` and `claude-code-plugin/` paths default to HTTP hooks.
...
| `powermem-codex-plugin/` | [SETUP.md](powermem-codex-plugin/SETUP.md) | [UNINSTALL.md](powermem-codex-plugin/UNINSTALL.md) | [README.md](powermem-codex-plugin/README.md) |
