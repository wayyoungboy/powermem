...
DEFAULT_LOG_FILE="$DATA_DIR/powermem-server."
DEFAULT_LOG_FILE="${DEFAULT_LOG_FILE}log"
LOG_FILE="${POWERMEM_LOG_FILE:-$DEFAULT_LOG_FILE}"
...
configure_local_no_proxy() {
  ...
  local_hosts="localhost,$loopback_v4,::1"
  if [ -n "${NO_PROXY:-}" ]; then
    NO_PROXY="$local_hosts,$NO_PROXY"
  else
...
write_runtime_base_url() {
  ...
  {
    write_runtime_var POWERMEM_BASE_URL "$base_url"
    write_runtime_var POWERMEM_ENV_FILE "$ENV_FILE"
    write_runtime_var POWERMEM_API_KEY "${POWERMEM_API_KEY:-}"
    write_runtime_var POWERMEM_USER_ID "${POWERMEM_USER_ID:-}"
    write_runtime_var POWERMEM_AGENT_ID "${POWERMEM_AGENT_ID:-}"
  } > "$tmp"
...
is_healthy() {
  ...
  if [ -n "${POWERMEM_API_KEY:-}" ]; then
    curl --noproxy '*' -fsS -m 5 -H "X-API-Key: $POWERMEM_API_KEY" "$url" 2>/dev/null | grep -q '"healthy"'
  else
    curl --noproxy '*' -fsS -m 5 "$url" 2>/dev/null | grep -q '"healthy"'
  fi
}
